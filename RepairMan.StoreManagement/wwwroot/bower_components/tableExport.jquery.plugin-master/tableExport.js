    ...
    $.fn.tableExport = function (options) {
        ...
        const defaults = {
            ...
            mso: {                            // MS Excel and MS Word related options
                ...
                rtl: true,                     // true = Set worksheet option 'DisplayRightToLeft'
                ...
                xlsx: {                         // Specific Excel 2007 XML format settings:
                    formatId: {                   // XLSX format (id) used to format excel cells. See readme.md: data-tableexport-xlsxformatid
                        ...
                        numbers: 0,                 // The default Format id, or a format string (e.g. '\"T\"\ #0.00'), or a function(cell, row, col)
                    },
                    format: {
        ...
        if (defaults.type === 'csv' || defaults.type === 'tsv' || defaults.type === 'txt') {
            ...
            _.each(fields, function (item) {
                if (item.field != "state" && item.field != "rowNumber") {
                    keyValue.push({
                        key: item.field,
                        value: item.title
                    });
            ...
            var $this, additionalParams, downloadTimer, generateForm, inputs, params, token, waitCount;
            params = {};
            params.__RequestVerificationToken = $("input[name=__RequestVerificationToken]").val();
            params.headers = keyValue;
            params.exportType = defaults.type;

            params.formName = window.document.title.replace("لیست ", "");
            ...
            $.ajax({
                type: "POST",
                url: this.data("exportUrl"),
                data: params
            }).done(function (data) {
                //Convert Base64 string to Byte Array.
                var bytes = Base64ToBytes(data.base64Encoded);
